...
# Target languages
# Target languages (expanded for Indian regional & coastal languages)
languages = [
    "hi",  # Hindi
    "ta",  # Tamil
    "te",  # Telugu
...
def read_js_file(filename):
    """Read and parse JavaScript screenTexts file"""
    try:
        with open(filename, 'r', encoding='utf-8') as f:
            content = f.read()
        
        # Extract the object part (everything after the = sign)
        match = re.search(r'export const screenTexts = ({.*});', content, re.DOTALL)
        if match:
            js_object = match.group(1)
            # Convert JS object to Python dict (basic conversion)
            # Replace JS object syntax with Python dict syntax
            js_object = js_object.replace('true', 'True').replace('false', 'False').replace('null', 'None')
            return eval(js_object)  # Note: eval is used here for simplicity
        else:
...
def translate_text(text, target_lang):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"❌ API Error for '{text}' → {target_lang}")
            return text
...
def dict_to_js_object(data, indent=0):
    ...
    if isinstance(data, dict):
        ...
        for i, (key, value) in enumerate(items):
            ...
            if isinstance(value, (dict, list)):
                lines.append(f"{spaces}{key}: {dict_to_js_object(value, indent + 1)}{comma}")
            elif isinstance(value, str):
                escaped_value = value.replace('"', '\\"').replace('\n', '\\n')
                lines.append(f'{spaces}{key}: "{escaped_value}"{comma}')
            else:
...
def main():
    ...
    if API_KEY == "YOUR_SARVAM_API_KEY":
        print("❌ Please set your actual Sarvam API key!")
        return
    
    # Read the original screenTexts.js file
    print("📖 Reading screenTexts.js...")
    screenTexts = read_js_file("screenTexts.js")
  ...
  "TripPlanningScreen": {
    ...
    for screen_name in screenTexts.keys():
        ...
        for lang in languages:
            ...
            try:
                screenTexts[screen_name][lang] = translate_dict(
                ...
                print(f"  ❌ Failed for {lang.upper()}: {e}")
    
    # Generate JavaScript file
    print("\n📝 Generating JavaScript file...")
    js_content = f"export const screenTexts = {dict_to_js_object(screenTexts)};"
    
    # Save to JavaScript file
    try:
        with open("screenTexts_multilingual.js", "w", encoding="utf-8") as f:
            f.write(js_content)
        print("✅ Multilingual screenTexts saved to screenTexts_multilingual.js")
        
        # Also save JSON version for backup
        with open("screenTexts_backup(1).json", "w", encoding="utf-8") as f:
            json.dump(screenTexts, f, ensure_ascii=False, indent=2)
        print("✅ JSON backup saved to screenTexts_backup(1).json")
        
        # Show sample
        print("\n📄 Sample output:")
        if "hi" in screenTexts.get("HomeScreen", {}):
            print(f"Hindi title: {screenTexts['HomeScreen']['hi']['title']}")
